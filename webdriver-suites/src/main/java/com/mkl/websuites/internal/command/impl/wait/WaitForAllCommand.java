...
/**
 * If both retryCount and retryTimeout are specified, the test will fail whenever any of this fail
 * condition is fist met.
 * 
 * @author m.klosinski
 *
 */
...
public class WaitForAllCommand extends ControlFlowHandler {
    ...
    protected List<SchemaValidationRule> defineValidationRules() {
        return Arrays.asList(new SchemaValidationRule(RETRY_TIMEOUT).addOptionalElements(RETRY_COUNT)
    ...
    protected List<ParameterValueValidator> defineParameterValueValidators() {
        return Arrays.asList((ParameterValueValidator) new IntegerNumberParamValidator(RETRY_COUNT, -1, MAX_VALUE),
                new IntegerNumberParamValidator(RETRY_TIMEOUT, -1, MAX_VALUE), new IntegerNumberParamValidator(
                        "retrypPause", 0, MAX_VALUE));
    }

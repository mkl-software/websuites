...
public class NegCheckSelectSelectedTextContainsCommand extends
		CheckSelectSelectedTextContainsCommand {

	public NegCheckSelectSelectedTextContainsCommand(
			Map<String, String> parameterMap) {
		super(parameterMap);
	}

	public NegCheckSelectSelectedTextContainsCommand(String selector,
			String expectedText) {
	...
	protected class NegCheckSelectSelectedTextContains extends CheckSelectSelectedTextContains {
		...
		protected void runSingleStringAssertion(StringAssert assertion, String foundText) {
			...
			assertion
				.overridingErrorMessage("Expecting selected option NOT to contain text '%s' in the SELECT "
						+ "element picked by selector '%s', but found selected option '%s'",
						expectedSelectText, by, foundText)
				.isEmpty();
		}
	}
